module UserItemVotesHelper
  def link_to_upvote(object)
    link_to 'Upvote', vote_item_path(object), method: :post, class: 'text-danger'
  end

  def link_to_downvote(object)
    link_to 'Downvote', vote_item_path(object), method: :delete, class: 'text-muted'
  end

  def render_votes_for_item(item = nil)
    if current_user && item.votes.map(&:user_id).include?(current_user.id)
